"""
AzamPay REST client — token caching, MNO checkout.
See AzamPay docs / sandbox portal for credentials.
"""
...
TOKEN_CACHE_KEY = "azampay_access_token_v1"
TOKEN_TTL_BUFFER_SEC = 120

# (connect timeout sec, read timeout sec) — sandbox MNO checkout can be slow; short read timeouts
# increase RemoteDisconnected / empty response rates.
_CHECKOUT_HTTP_TIMEOUT = (20, 90)
_CHECKOUT_TRANSPORT_RETRIES = 2
_CHECKOUT_RETRY_SLEEP_SEC = 1.5


def normalize_tz_msisdn(phone: str) -> str:
    """Expect AzamPay format: 255XXXXXXXXX (no leading +)."""
...
class AzamPayClient:
    """Thin wrapper around AzamPay sandbox/production APIs."""
    ...
    def __init__(self) -> None:
        ...
        self.callback_url = (getattr(settings, "AZAMPAY_CALLBACK_URL", None) or "").rstrip("/")
        self.x_api_key = getattr(settings, "AZAMPAY_X_API_KEY", None) or ""

        if not all([self.auth_base, self.api_base, self.app_name, self.client_id, self.client_secret, self.callback_url]):
            raise AzamPayConfigurationError(
                "AzamPay is not configured. Set AZAMPAY_AUTH_BASE_URL, AZAMPAY_API_BASE_URL, "
                "AZAMPAY_APP_NAME, AZAMPAY_CLIENT_ID, AZAMPAY_CLIENT_SECRET, AZAMPAY_CALLBACK_URL"
            )
        ...
        for attempt in range(1, _CHECKOUT_TRANSPORT_RETRIES + 1):
            try:
                ...
                if attempt < _CHECKOUT_TRANSPORT_RETRIES:
                    logger.warning(
                        "AzamPay MNO checkout transport error (attempt %s/%s), retrying in %ss: %s",
                        attempt,
                        _CHECKOUT_TRANSPORT_RETRIES,
                        _CHECKOUT_RETRY_SLEEP_SEC,
                        e,
                    )
